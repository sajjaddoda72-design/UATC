...
logger = logging.getLogger("UATC_v3.2")
...
class AdaptiveExpertController:
    ...
    def _update_phase_machine(self, s: ModelState, paradigm: str) -> None:
        ...
        if self._phase == "WARMUP":
            ...
            is_acceleration_stable = self._mem_accel_ema < 1e-4
            ...
            if (not s.oom_detected and is_memory_safe
            ...
            if (self._steps_in_phase >= min_steps
                ...
                logger.info("Phase RECOVERY->SCALING at step %d", s.step)

    # ------------------------------------------------------------------
    # 3-state Schmitt trigger
    # ------------------------------------------------------------------
    def _update_bs_state(self, z_raw: float) -> None:
        """3-state Schmitt: STABLE / WATCH / REDUCED with hysteresis."""
        c = self.cfg
        if self._bs_state is None:
            self._bs_state = "STABLE"
        thr_low = c.memory_soft_limit * 0.70
        thr_mid = c.memory_soft_limit * 0.80
        thr_high = c.memory_soft_limit * 0.88

        if z_raw < thr_low:
            self._bs_state = "STABLE"
        elif z_raw > thr_high:
            self._bs_state = "REDUCED"
        elif self._bs_state == "STABLE" and z_raw > thr_mid:
            self._bs_state = "WATCH"
        elif self._bs_state == "WATCH" and z_raw < thr_low:
            self._bs_state = "STABLE"
        elif self._bs_state == "WATCH" and z_raw > thr_high:
            self._bs_state = "REDUCED"
        elif self._bs_state == "REDUCED" and z_raw < thr_mid:
            self._bs_state = "WATCH"
    ...
    def decide(self, state: ModelState) -> ControllerAction:
        ...
        # J. BS multiplier clamping per phase
        ...
        if self._bs_state == "WATCH":
            bs_clamp_hi = bs_clamp_lo + (bs_clamp_hi - bs_clamp_lo) * 0.5
        ...
        proposed_bs_pre_slew = max(
            c.min_batch_size,
            int(round(int(s.current_batch_size * bs_multiplier) / 16.0)) * 16,
        )
