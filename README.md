...
where $\mathrm{span} = \mathrm{HardLimit} - \mathrm{SoftLimit}$ and $\xi$ is the paradigm elasticity from §3.7. The final gains multiply the base gains by $\mathrm{capacity\_scale} \cdot \{\mathrm{gain}\}$. The integral term uses *conditional integration* (anti-windup): when the PID output saturates against a clamp bound, the integrator is bled in the opposite direction by an amount proportional to $(\mathrm{unclamped} - \mathrm{clamped}) \cdot \mathrm{anti\_windup\_gain}$ rather than being accumulated, preventing integral windup that would otherwise cause post-saturation overshoot.
