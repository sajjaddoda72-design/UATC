...
Inside your main training loop, wrap the forward and backward passes in a standard try-except block to capture hardware memory alerts. Build the ModelState telemetry snapshot at the end of each step, query the controller for directives, and apply them immediately to the next iteration.
```python
import torch
...
while step <= total_steps:
    ...
    try:
        # 3. Autocast forward pass under adaptive AMP configurations
        with torch.amp.autocast('cuda', enabled=current_amp):
            outputs = model(input_ids=input_ids, attention_mask=attention_mask)
            logits = outputs.logits
            
            # Compute causal per-token cross entropy
