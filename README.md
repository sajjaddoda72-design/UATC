...
GPU memory telemetry is noisy due to asynchronous CUDA execution, allocator fragmentation, and driver-level caching. UATC runs a discrete Kalman filter on the raw pressure signal `z_k ∈ [0, 1]`:
...
Default `Q = 1e-4` (process noise), `R = 5e-4` (measurement noise). The smoothed state is used to compute memory velocity and acceleration through Exponential Moving Averages (EMAs), which feed the PID derivative term and the dynamic setpoint backoff.
...
```
Setpointₖ = SoftLimit × (0.95 − Clamp(aₖ × 50, 0, 0.10))
eₖ = Setpointₖ − zₖ
```

Gains are dynamically scaled by three factors: a *proximity gain* that ramps as pressure approaches the soft limit, an *acceleration gain* that ramps with rising `|aₖ|`, and an *elasticity gain* `ξ` that depends on the training paradigm (see §3.7). The integral term is clamped (`|I| ≤ 0.4`) and uses conditional integration (anti-windup): when the output saturates, the integrator is bled in the opposite direction rather than accumulated.

### 3.4 Delay Compensation (Smith Predictor)

Batch-size changes do not reflect in VRAM telemetry for several steps due to kernel launch latency and asynchronous allocator commits. The Smith predictor compensates by maintaining a delay-line buffer of past PID outputs and adding a bounded correction term to the current output:

```
ŷ_smith = uₖ + λ · (uₖ − v̂ₖ),   |correction| ≤ 0.05
```

with `λ = 0.1` and a configurable delay of 4–12 steps. This prevents the PID from over-correcting during the dead-time window.
